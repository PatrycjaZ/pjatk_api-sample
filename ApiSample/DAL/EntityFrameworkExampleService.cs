...
namespace ApiSample.DAL
{
    ...
    {
        private readonly API_hotelContext context;
        public EntityFrameworkExampleService(API_hotelContext _context)
        {
            context = _context;
        }
        public bool AddGuest(GuestRequestDto newGuest)
        {
            try
            {
                ...
                context.SaveChanges(); // zapisujemy zmiany w bazie (odpowiednik wykonania napisanego wcześniej polecenia)
        ...
        public ICollection<Gosc> GetGuestsCollectionWithReservations(string lastName)
        {
            return context.Gosc // metoda działająca na klasach modelowych opartych bezpośrednio o tabele w bazie. 
                .Where(x => x.Nazwisko == lastName)
                .Include(x => x.Rezerwacja)
                .ThenInclude(x => x.NrPokojuNavigation) // odniesienie do pokoju
                .ThenInclude(x => x.IdKategoriaNavigation) // odniesienie do kategorii
                .ToList();
        }
